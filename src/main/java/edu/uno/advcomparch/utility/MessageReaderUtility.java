...
public class MessageReaderUtility {
    ...
    public List<Message<L1InMessage>> readMessages(String filename) throws FileNotFoundException {
        var messages = new ArrayList<Message<L1InMessage>>();
        ...
        while (messageScanner.hasNextLine()) {
            ...
            if (cpuMessageType == CPUMessageType.CPU_READ) {
                messages.add(MessageBuilder.withPayload(L1InMessage.CPUREAD)
                ...
                messages.add(MessageBuilder.withPayload(L1InMessage.CPUWRITE)
                        .setHeader("source", "CPU")
                        .setHeader("address", messageSplit[1])
